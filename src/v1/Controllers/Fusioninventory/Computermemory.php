      ...
      if ($this->vs['NUMSLOTS']->isValid($contentMemory))
      {
        ...
        if (is_null($slot->memorymodule))
        {
          ...
          if (
              ...
              $slot->memorymodule->devicememorytype_id === $typeId &&
              $slot->memorymodule->manufacturer_id === $manufacturerId &&
              $slot->memorymodule->devicememorymodel_id === $modelId
          )
          {
            // we are OK
        ...
        if ($findAndAttach)
        {
          if (is_null($serial))
          {
            $memorymodule = \App\Models\Memorymodule::
                where('serial', $serial)
              ->where('size', $size)
              ->where('devicememorytype_id', $typeId)
              ->where('manufacturer_id', $manufacturerId)
              ->where('devicememorymodel_id', $modelId)
          ...
          if (is_null($memorymodule))
          {
            ...
            $memorymodule->devicememorymodel_id = $modelId;
            $memorymodule->devicememorytype_id = $typeId;
