...
final class ItemsDevicegenericsMigration extends AbstractMigration
{
  public function change()
  {
    ...
    $item = $this->table('item_devicegeneric');

    if ($this->isMigratingUp())
    {
      $stmt = $pdo->query('SELECT * FROM glpi_items_devicegenerics');
      $rows = $stmt->fetchAll();
      foreach ($rows as $row)
      {
        $data = [
      ...
      {
        $this->execute("SELECT setval('item_devicegeneric_id_seq', (SELECT MAX(id) FROM item_devicegeneric)+1)");
      }
    } else {
      // rollback
      $item->truncate();
